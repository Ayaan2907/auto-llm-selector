...
If you only want to **use the published package** in your app, start with the [README](README.md) section **Use in your application**. To **see the router end-to-end** without publishing locally, use the [README](README.md) section **Run the demo (`sample.ts`)**.
...
- An **OpenRouter API key** for flows that call OpenRouter or load embeddings (e.g. `sample.ts`, integration-style checks).
...
Semantic features depend on `@tensorflow/tfjs-node`. If `pnpm install` skipped optional build scripts, you may see errors about `tfjs_binding.node` when you run tests or `sample.ts`.
