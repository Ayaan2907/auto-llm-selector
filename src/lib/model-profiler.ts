 ...
 */
const KNOWN_MODEL_PROFILES: Record<string, Partial<ModelCapabilities & ModelCharacteristics>> = {
    // OpenAI Models
    'openai/gpt-4o': {
        coding: 0.95, creative: 0.90, analytical: 0.92, reasoning: 0.95, conversational: 0.90, general: 0.95,
        speedTier: 'medium', accuracyTier: 'excellent', isReasoning: true, isMultimodal: true
    },
    'openai/gpt-4o-mini': {
        coding: 0.85, creative: 0.80, analytical: 0.82, reasoning: 0.85, conversational: 0.85, general: 0.85,
        speedTier: 'fast', accuracyTier: 'high', isReasoning: true, isMultimodal: true
    },
    'openai/gpt-4-turbo': {
        coding: 0.90, creative: 0.88, analytical: 0.90, reasoning: 0.92, conversational: 0.88, general: 0.90,
        speedTier: 'medium', accuracyTier: 'excellent', isReasoning: true, isMultimodal: true
    },
    'openai/gpt-3.5-turbo': {
        coding: 0.75, creative: 0.70, analytical: 0.72, reasoning: 0.75, conversational: 0.80, general: 0.75,
        speedTier: 'fast', accuracyTier: 'good', isReasoning: false, isMultimodal: false
    },

    // Anthropic Models  
    'anthropic/claude-3-opus': {
        coding: 0.92, creative: 0.95, analytical: 0.95, reasoning: 0.95, conversational: 0.95, general: 0.93,
        speedTier: 'slow', accuracyTier: 'excellent', isReasoning: true, isMultimodal: true
    },
    'anthropic/claude-3-sonnet': {
        coding: 0.88, creative: 0.90, analytical: 0.90, reasoning: 0.90, conversational: 0.90, general: 0.88,
        speedTier: 'medium', accuracyTier: 'excellent', isReasoning: true, isMultimodal: true
    },
    'anthropic/claude-3-haiku': {
        coding: 0.75, creative: 0.80, analytical: 0.78, reasoning: 0.80, conversational: 0.85, general: 0.78,
        speedTier: 'ultra-fast', accuracyTier: 'good', isReasoning: true, isMultimodal: true
    },

    // Google Models
    'google/gemini-pro-1.5': {
        coding: 0.85, creative: 0.82, analytical: 0.88, reasoning: 0.85, conversational: 0.80, general: 0.85,
        speedTier: 'medium', accuracyTier: 'high', isReasoning: true, isMultimodal: true
    },
    'google/gemini-flash-1.5': {
        coding: 0.80, creative: 0.75, analytical: 0.80, reasoning: 0.78, conversational: 0.75, general: 0.78,
        speedTier: 'ultra-fast', accuracyTier: 'good', isReasoning: true, isMultimodal: true
    },

    // Meta Models
    'meta-llama/llama-3.1-405b': {
        coding: 0.88, creative: 0.85, analytical: 0.85, reasoning: 0.88, conversational: 0.82, general: 0.85,
        speedTier: 'slow', accuracyTier: 'high', isReasoning: true, isMultimodal: false
    },
    'meta-llama/llama-3.1-70b': {
        coding: 0.82, creative: 0.80, analytical: 0.80, reasoning: 0.82, conversational: 0.78, general: 0.80,
        speedTier: 'medium', accuracyTier: 'high', isReasoning: true, isMultimodal: false
    },
    'meta-llama/llama-3.1-8b': {
        coding: 0.70, creative: 0.65, analytical: 0.68, reasoning: 0.70, conversational: 0.70, general: 0.68,
        speedTier: 'fast', accuracyTier: 'good', isReasoning: true, isMultimodal: false
    },

    // Free/OSS Models
    'gryphe/mythomist-7b:free': {
        coding: 0.60, creative: 0.75, analytical: 0.50, reasoning: 0.55, conversational: 0.70, general: 0.60,
        speedTier: 'fast', accuracyTier: 'basic', isReasoning: false, isMultimodal: false
    },
    'microsoft/wizardlm-2-8x22b': {
        coding: 0.78, creative: 0.75, analytical: 0.75, reasoning: 0.80, conversational: 0.75, general: 0.75,
        speedTier: 'medium', accuracyTier: 'good', isReasoning: true, isMultimodal: false
    }
};
...
export class ModelProfiler {
    ...
    static createModelProfile(modelInfo: ModelInfo): ModelProfile {
        ...
        const characteristics = this.buildCharacteristics(modelInfo, knownProfile, provider, modelFamily);

        // Calculate profile confidence
        const profileConfidence = knownProfile ? 0.95 : this.calculateInferredConfidence(modelInfo);
        ...
        logger.info(`Created profile for ${modelInfo.id} (confidence: ${profileConfidence.toFixed(2)})`);
        return profile;
    }
    ...
    static rankModelsForCategory(
        ...
        const rankedModels = models
            ...
            .map(model => {
                const score = this.calculateCategoryScore(model, category, requirements);
        ...
        logger.info(`Ranked ${rankedModels.length} suitable models for ${category}`);
    ...
    static getBestModelForCategory(
        ...
        requirements?: Parameters<typeof ModelProfiler.rankModelsForCategory>[2]
            ...
            return ranking.rankedModels[.length > 0 ? ranking.rankedModels[0].model : null;]
    }
    ...
    private static buildCapabilitiesFromKnown(knownProfile: Partial<ModelCapabilities>): ModelCapabilities {
        return {
    ...
    private static inferCapabilities(
        ...
        let coding = 0.5, creative = 0.5, analytical = 0.5, reasoning = 0.5, conversational = 0.5, general = 0.5;

        // Provider-based adjustments
        switch (provider) {
            case 'openai':
                coding += 0.2; reasoning += 0.15; general += 0.1;
                break;
            case 'anthropic':
                creative += 0.2; conversational += 0.2; reasoning += 0.15;
                break;
            case 'google':
                analytical += 0.15; general += 0.1;
                break;
            case 'meta-llama':
                coding += 0.1; reasoning += 0.1;
                break;
        }

        // Model family adjustments
        if (modelFamily.includes('gpt-4')) {
            coding += 0.15; reasoning += 0.2;
        } else if (modelFamily.includes('claude-3')) {
            creative += 0.15; conversational += 0.15;
        }

        // Cost-based inference (expensive models tend to be better)
        const avgCost = (parseFloat(modelInfo.pricing.prompt) + parseFloat(modelInfo.pricing.completion)) / 2;
    ...
    private static buildCharacteristics(
        ...
        if (knownProfile) {
            return {
                ...
                accuracyTier: knownProfile.accuracyTier ?? this.inferAccuracyTier(modelInfo, provider),
                contextTier: this.inferContextTier(modelInfo.context_length),
                provider,
                modelFamily,
                isReasoning: knownProfile.isReasoning ?? this.inferReasoningCapability(modelFamily),
                isMultimodal: knownProfile.isMultimodal ?? this.inferMultimodalCapability(modelInfo.id)
            };
        }
    ...
    private static inferSpeedTier(modelInfo: ModelInfo): ModelCharacteristics['speedTier'] {
        const id = modelInfo.id.toLowerCase();
        
        if (id.includes('turbo') || id.includes('flash') || id.includes('haiku') || id.includes('mini')) {
            return 'ultra-fast';
        }
    ...
    private static inferCostTier(modelInfo: ModelInfo): ModelCharacteristics['costTier'] {
        const avgCost = (parseFloat(modelInfo.pricing.prompt) + parseFloat(modelInfo.pricing.completion)) / 2;
    ...
    private static inferAccuracyTier(modelInfo: ModelInfo, provider: string): ModelCharacteristics['accuracyTier'] {
        const id = modelInfo.id.toLowerCase();
    ...
    private static inferContextTier(contextLength: number): ModelCharacteristics['contextTier'] {
    ...
    private static inferMultimodalCapability(modelId: string): boolean {
        const id = modelId.toLowerCase();
        return id.includes('vision') || id.includes('gpt-4') || id.includes('claude-3') || 
               id.includes('gemini') || id.includes('gpt-4o');
    }
    ...
    private static meetsRequirements(
        ...
        if (requirements.maxCost && (model.promptCostPerToken > requirements.maxCost)) {
            return false;
        }
        ...
        if (requirements.minSpeed) {
            ...
            const modelSpeedIndex = speedOrder.indexOf(model.characteristics.speedTier);
        ...
        if (requirements.minAccuracy) {
            ...
            const modelAccuracyIndex = accuracyOrder.indexOf(model.characteristics.accuracyTier);
    ...
    private static calculateCategoryScore(
        ...
        if (requirements?.maxCost) {
            ...
            if (costRatio <= 0.5) score += 0.1; // Bonus for being well under budget
        ...
        if (category === PromptType.Reasoning && model.characteristics.isReasoning) {
            score += 0.1;
        }
    ...
    private static generateScoreReasoning(
        ...
        reasons.push(`${(capabilityScore * 100).toFixed(0)}% ${category} capability`);
        ...
        if (model.characteristics.costTier === 'free' || model.characteristics.costTier === 'cheap') {
            reasons.push('cost-effective');
        }

        if (model.characteristics.contextTier === 'huge' || model.characteristics.contextTier === 'large') {
