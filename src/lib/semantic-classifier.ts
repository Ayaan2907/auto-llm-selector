...
export class SemanticClassifier {
  ...
  private async performInitialization(): Promise<void> {
    ...
    logger.info(
      `Semantic classifier ready with ${stats.initializedCategories} category embeddings`
    );
  }
  ...
  async classifyPrompt(prompt: string): Promise<SemanticClassificationResult> {
    ...
    try {
      ...
      const referenceEmbeddings =
        referenceEmbeddingManager.getAllReferenceEmbeddings();
      ...
      logger.debug(
        `Semantic classification completed in ${processingTime}ms: ${category.type} (${category.confidence.toFixed(3)})`
      );
  ...
  private calculateSimilarities(
    ...
    for (const [category, referenceEmbedding] of referenceEmbeddings) {
      try {
        const similarity = semanticEmbedder.cosineSimilarity(
          promptEmbedding,
          referenceEmbedding
        );
   ...
   */
  private findBestMatch(similarities: Map<string, number>): {
    category: string;
    similarity: number;
  } {
    let bestCategory = '';
  ...
  private createCategoryResult(
    ...
    if (similarity < SEMANTIC_CONFIG.MIN_SIMILARITY_THRESHOLD) {
      logger.debug(
        `Low similarity (${similarity.toFixed(4)}) - falling back to general category`
      );
      return {
  ...
  private calculateConfidence(
    ...
    const similarities = Array.from(allSimilarities.values()).sort(
      (a, b) => b - a
    );
    let uniqueness = 1.0;
    ...
    const scaledConfidence =
      SEMANTIC_CONFIG.CONFIDENCE_BASE +
      combinedConfidence * SEMANTIC_CONFIG.CONFIDENCE_SCALE;

    return Math.max(
      0.1,
      Math.min(scaledConfidence, SEMANTIC_CONFIG.MAX_CONFIDENCE)
    );
  }
