...
import { semanticEmbedder } from './lib/semantic-embedder.js'
...
export class ReferenceEmbeddingManager {
    ...
    private async createCategoryEmbedding(category: PromptType): Promise<void> {
        try {
            ...
            if (existingEmbedding && existingEmbedding.length > 0) {
                logger.debug(`Reference embedding for ${category} already exists in cache`)
                return
            }
            ...
            for (const text of referenceTexts) {
                try {
                    ...
                    logger.warn(`Failed to generate embedding for reference text in ${category}:`, error)
                    continue
                }
            }

            if (embeddings.length === 0) {
                throw new Error(`No valid embeddings generated for category ${category}`)
            }
            ...
            logger.info(`Created reference embedding for ${category} (${embeddings.length} samples)`)
            
        } catch (error) {
            logger.error(`Failed to create reference embedding for ${category}:`, error)
            throw error
        }
    }
     ...
     */
    async addCustomReferenceTexts(category: PromptType, texts: string[]): Promise<void> {
        logger.info(`Adding ${texts.length} custom reference texts for ${category}`)
        ...
        if (newEmbeddings.length === 0) {
            logger.warn(`No valid embeddings generated from custom texts for ${category}`)
            return
        }
        ...
        if (existingEmbedding) {
            ...
            logger.info(`Updated reference embedding for ${category} with ${newEmbeddings.length} custom samples`)
        } else {
            ...
            logger.info(`Created new reference embedding for ${category} from ${newEmbeddings.length} custom samples`)
        }
    }
